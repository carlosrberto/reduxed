import { toSnakeCase, getActionType, isObject, isFunction } from './helpers'
...
describe('getActionType', () => {
  it('should return the prefix concatenated with name', () => {
    expect(getActionType('foo/bar', 'fooBarAction')).toEqual(
      'foo/bar/FOO_BAR_ACTION'
    )
  })

  it('should return only the name if prefix is invalid', () => {
    expect(getActionType('', 'fooBarAction')).toEqual('FOO_BAR_ACTION')

    expect(getActionType(undefined, 'fooBarAction')).toEqual('FOO_BAR_ACTION')
  })
})
