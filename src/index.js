...
export const create = (...handlers: Array<Handler>) => (
  initialState: any,
  options: Options = { typePrefix: '' }
): ReducerDescriptor => {
  const handlersByType: { [string]: ActionHandler } = handlers.reduce(
    (acc, { name, fn }) => {
...
export const handler = (name: string, fn: ActionHandler): Handler => ({
  name,
  fn,
})
...
export const getReducer = ({
  handlersByType,
  initialState,
}: ReducerDescriptor): ReduxReducer => (state, action) => {
...
export const withScope = (scope: any, target: any) => {
  if (target !== undefined) {
    ...
    throw Error(
      'Invalid scope target. It should be a reducer function or an object containing action creators'
    )
  } else {
